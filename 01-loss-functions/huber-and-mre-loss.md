# Loss Functions in Deep Learning — Huber & MRE
...
Part C (MSE) and a companion note on the imbalance-mitigation mechanisms (StratSample, LDS, RankSim, ConR) are natural next additions — see [What's Next](#d-whats-next-in-this-learning-hub).
...
## C. Cross-Comparison Summary

| | MSE | Huber (δ=0.15) | MRE (ε=0.001) |
|---|---|---|---|
| What it measures | Absolute squared error | Absolute error, bounded | Relative (%) error |
...
| Emphasizes | Large-magnitude errors equally everywhere | Same, but caps outlier influence | Small-target (majority-class) accuracy |

---

## D. What's Next in This Learning Hub

This note currently covers Huber and MRE in full depth. Natural next additions, in the same format:

1. **Part C — MSE** (the baseline; the cross-comparison table above already previews its numbers).
2. A companion note on the four **imbalance-mitigation mechanisms**: StratSample (stratified sampling), LDS (Label Distribution Smoothing), RankSim, and ConR — including a novel ConR+LDS combination.
3. A short primer on **AdamW** (the optimizer actually used vs. plain SGD used above for teaching clarity).
